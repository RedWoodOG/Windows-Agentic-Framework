...
public sealed class TaskManager
{
    ...
    private readonly ConcurrentDictionary<string, A9NTask> _tasks = new();
    ...
    public A9NTask? GetTask(string taskId) => _tasks.TryGetValue(taskId, out var task) ? task : null;

    public A9NTask GetTaskOrThrow(string taskId) =>
        _tasks.TryGetValue(taskId, out var task) ? task : throw new TaskNotFoundException(taskId);

    public List<A9NTask> ListTasks() => _tasks.Values.OrderByDescending(t => t.CreatedAt).ToList();
    ...
    public List<A9NTask> GetOrderedTasks()
    {
    ...
    public List<A9NTask> GetOverdueTasks()
    {
    ...
    private void LoadTasks()
    {
        ...
        foreach (var file in Directory.EnumerateFiles(_tasksDir, "*.json"))
        {
            try
            {
                ...
                var task = JsonSerializer.Deserialize<A9NTask>(json, JsonOpts);
    ...
    private Task SaveTaskAsync(A9NTask task, CancellationToken ct)
    {
...
{
    public required List<A9NTask> BlockedBy { get; init; }
    public required List<A9NTask> Blocking { get; init; }
}
