...
description: Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration via Python. Uses OAuth2 with automatic token refresh. No external binaries needed — runs entirely with Google's Python client libraries in the A9N venv.
version: 1.0.0
...
```bash
GSETUP="python ~/.a9n/skills/productivity/google-workspace/scripts/setup.py"
```
...
- Token is stored at `~/.a9n/google_token.json` and auto-refreshes.
- Pending OAuth session state/verifier are stored temporarily at `~/.a9n/google_oauth_pending.json` until exchange completes.
...
```bash
GAPI="python ~/.a9n/skills/productivity/google-workspace/scripts/google_api.py"
```
