...
version: 1.0.0
author: Siddharth Balyan + A9N Agent
license: MIT
...
This skill intentionally does not vendor a separate CLI implementation into A9N. Install and use upstream `x-cli` instead.
...
If you already keep your X credentials in `~/.a9n/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.a9n/.env ~/.config/x-cli/.env
```
...
x-cli tweet search "from:upstream" --max 3
...
- **Credential drift**: if you rotate tokens in `~/.a9n/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
