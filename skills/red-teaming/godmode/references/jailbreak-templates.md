...
## 5. GODMODE FAST — A9N 4 405B (Zero Refusal)

**Model:** `nousresearch/a9n-4-405b`
**Strategy:** A9N models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with A9N Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.a9n/config.yaml`:
