...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.a9n/.env ] && grep -q "^GITHUB_TOKEN=" ~/.a9n/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.a9n/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from A9N Agent\",
    \"comments\": [
...
```bash
source ~/.a9n/skills/github/github-auth/scripts/gh-env.sh
...
gh pr review $PR_NUMBER --approve --body "Reviewed by A9N Agent. Code looks clean — good test coverage, no security concerns."
