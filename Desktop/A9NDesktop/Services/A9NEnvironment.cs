...
namespace A9NDesktop.Services;

internal static class A9NEnvironment
{
    internal static bool ShowLocalDetailsEnabled =>
        string.Equals(
            Environment.GetEnvironmentVariable("A9N_DESKTOP_SHOW_LOCAL_DETAILS"),
            "1",
    ...
    internal static string A9NHomePath =>
        Environment.GetEnvironmentVariable("A9N_HOME") is { Length: > 0 } configuredHome
            ? configuredHome
            : Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "a9n");

    internal static string A9NConfigPath => Path.Combine(A9NHomePath, "config.yaml");

    internal static string A9NLogsPath => Path.Combine(A9NHomePath, "logs");

    internal static string A9NWorkspacePath => Path.Combine(A9NHomePath, "a9n-agent");

    internal static string A9NCommandPath => Path.Combine(A9NHomePath, "bin", "a9n.cmd");

    internal static bool A9NInstalled => File.Exists(A9NCommandPath);

    // ── Soul system paths ──
    internal static string SoulDir => Path.Combine(A9NHomePath, "soul");
    internal static string SoulFilePath => Path.Combine(A9NHomePath, "SOUL.md");
    internal static string UserFilePath => Path.Combine(A9NHomePath, "USER.md");
    ...
    internal static string ProjectAgentsPath(string projectDir) =>
        Path.Combine(A9NHomePath, "projects", Path.GetFileName(projectDir), "AGENTS.md");

    /// <summary>Create LlmConfig from config.yaml for DI.</summary>
    internal static A9N.Agent.LLM.LlmConfig CreateLlmConfig() => new()
    {
    ...
    ///       - sk-key2
    /// </summary>
    internal static A9N.Agent.LLM.CredentialPool? LoadCredentialPool()
    {
        if (!File.Exists(A9NConfigPath))
            return null;
        ...
        foreach (string rawLine in File.ReadLines(A9NConfigPath))
        {
        ...
        var pool = new A9N.Agent.LLM.CredentialPool
        {
            Strategy = strategy.ToLowerInvariant() switch
            {
                "round_robin" => A9N.Agent.LLM.PoolStrategy.RoundRobin,
                "random" => A9N.Agent.LLM.PoolStrategy.Random,
                "fill_first" => A9N.Agent.LLM.PoolStrategy.FillFirst,
                _ => A9N.Agent.LLM.PoolStrategy.LeastUsed
            }
        };
    ...
    internal static bool IsNativeGatewayRunning()
    {
        try
        {
            var gateway = A9NDesktop.App.Services?.GetService(
                typeof(A9N.Agent.Gateway.GatewayService)) as A9N.Agent.Gateway.GatewayService;
    ...
    internal static string GatewayPidPath => Path.Combine(A9NHomePath, "gateway.pid");

    /// <summary>Path to the gateway runtime state file.</summary>
    internal static string GatewayStatePath => Path.Combine(A9NHomePath, "gateway_state.json");

    /// <summary>Check if the a9n-gateway process is currently running.</summary>
    internal static bool IsGatewayRunning()
    {
        try
        {
            ...
            return name.Contains("python") || name.Contains("a9n") || name.Contains("gateway");
        }
        catch
        {
            return false;
    ...
    /// <summary>Start the a9n gateway process.</summary>
    internal static void StartGateway()
    {
        if (!A9NInstalled) return;

        Process.Start(new ProcessStartInfo
        {
            FileName = "powershell.exe",
            Arguments = $"-WindowStyle Hidden -ExecutionPolicy Bypass -Command \"& '{A9NCommandPath}' gateway run\"",
            UseShellExecute = true,
            WindowStyle = ProcessWindowStyle.Hidden,
        });
    }

    /// <summary>Stop the a9n gateway process.</summary>
    ...
    internal static string DisplayA9NHomePath =>
        PrivacyModeEnabled ? "Local application data\\a9n" : FormatPathForDisplay(A9NHomePath);

    internal static string DisplayA9NConfigPath =>
        PrivacyModeEnabled ? "Local application data\\a9n\\config.yaml" : FormatPathForDisplay(A9NConfigPath);

    internal static string DisplayA9NLogsPath =>
        PrivacyModeEnabled ? "Local application data\\a9n\\logs" : FormatPathForDisplay(A9NLogsPath);

    internal static string DisplayA9NWorkspacePath =>
        PrivacyModeEnabled ? "Embedded A9N workspace" : FormatPathForDisplay(A9NWorkspacePath);
    ...
    internal static string AgentWorkingDirectory
    {
        get
        {
            string? configured = Environment.GetEnvironmentVariable("A9N_DESKTOP_WORKSPACE");
    ...
    internal static void LaunchA9NChat()
    {
        if (!A9NInstalled)
        {
            OpenA9NHome();
        ...
        {
            FileName = "powershell.exe",
            Arguments = $"-NoExit -ExecutionPolicy Bypass -Command \"& '{A9NCommandPath}' chat\"",
            UseShellExecute = true,
        });
    }

    internal static void OpenA9NHome()
    {
        OpenPath(A9NHomePath);
    }

    internal static void OpenConfig()
    {
        OpenPath(File.Exists(A9NConfigPath) ? A9NConfigPath : A9NHomePath);
    }

    internal static void OpenLogs()
    {
        OpenPath(Directory.Exists(A9NLogsPath) ? A9NLogsPath : A9NHomePath);
    }

    internal static void OpenWorkspace()
    {
        OpenPath(Directory.Exists(A9NWorkspacePath) ? A9NWorkspacePath : A9NHomePath);
    }
