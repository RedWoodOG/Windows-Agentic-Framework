...
/// <summary>
/// Pure C# chat service — bridges the WinUI frontend to the A9N Agent core.
/// No Python sidecar. Direct in-process agent execution.
/// </summary>
internal sealed class A9NChatService : IDisposable
{
    private readonly Agent _agent;
    private readonly IChatClient _chatClient;
    private readonly TranscriptStore _transcriptStore;
    private readonly ILogger<A9NChatService> _logger;

    private Session? _currentSession;
    private CancellationTokenSource? _streamCts;
    private bool _disposed;

    public A9NChatService(
        Agent agent,
        IChatClient chatClient,
        TranscriptStore transcriptStore,
        ILogger<A9NChatService> logger)
    {
        _agent = agent;
    ...
    public async Task<A9NChatReply> SendAsync(string message, CancellationToken ct)
    {
        EnsureSession();
        ...
        try
        {
            ...
            return new A9NChatReply(response, _currentSession.Id);
        }
        catch
        {
        ...
        [EnumeratorCancellation] CancellationToken ct)
    {
        ...
        try
        {
            await foreach (var evt in _agent.StreamChatAsync(message, _currentSession!, _streamCts.Token))
            {
                switch (evt)
                {
                    case A9N.Agent.LLM.StreamEvent.TokenDelta td:
                    ...
                    case A9N.Agent.LLM.StreamEvent.ThinkingDelta tk:
                        yield return new ChatStreamEvent(ChatStreamEventType.Thinking, tk.Text);
                        break;

                    case A9N.Agent.LLM.StreamEvent.StreamError err:
    ...
    internal sealed record A9NChatReply(string Response, string SessionId);
}
