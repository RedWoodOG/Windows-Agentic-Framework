...
using A9N.Agent.Memory;
using A9N.Agent.Skills;
using A9N.Agent.Permissions;
using A9N.Agent.Tasks;
using A9N.Agent.Buddy;
using A9N.Agent.Context;
using A9N.Agent.Agents;
using A9N.Agent.Coordinator;
using A9N.Agent.Mcp;
using A9N.Agent.Analytics;
using A9N.Agent.Plugins;
using A9N.Agent.Soul;
using A9N.Agent.Tools;
...
public partial class App : Application
{
    ...
    private static ServiceProvider ConfigureServices()
    {
        ...
        var logsDir = Path.Combine(A9NEnvironment.A9NHomePath, "a9n-data", "logs");
        Directory.CreateDirectory(logsDir);
        services.AddLogging(builder =>
        {
            builder.AddDebug();
            builder.SetMinimumLevel(LogLevel.Information);
            builder.AddProvider(new FileLoggerProvider(Path.Combine(logsDir, "a9n.log")));
        });

        // LLM config from environment/config.yaml
        var llmConfig = A9NEnvironment.CreateLlmConfig();
        ...
        var credentialPool = A9NEnvironment.LoadCredentialPool();
        ...
        // A9N home directory — ensure all required dirs exist on startup
        var a9nHome = A9NEnvironment.A9NHomePath;
        var projectDir = Path.Combine(a9nHome, "a9n-data");
        foreach (var dir in new[]
        {
            a9nHome, projectDir,
            Path.Combine(a9nHome, "soul"),              // mistakes.jsonl, habits.jsonl
        ...
        var soulPath = Path.Combine(a9nHome, "SOUL.md");
        var userPath = Path.Combine(a9nHome, "USER.md");
        ...
        System.Diagnostics.Debug.WriteLine($"A9N home: {a9nHome}");
        ...
        var wikiConfig = new A9N.Agent.Wiki.WikiConfig();
        services.AddSingleton(wikiConfig);
        services.AddSingleton<A9N.Agent.Wiki.IWikiStorage>(sp =>
            new A9N.Agent.Wiki.LocalWikiStorage(sp.GetRequiredService<A9N.Agent.Wiki.WikiConfig>()));
        services.AddSingleton(sp => new A9N.Agent.Wiki.WikiSearchIndex(
            Path.Combine(sp.GetRequiredService<A9N.Agent.Wiki.WikiConfig>().WikiPath, ".wiki-search.db"),
            sp.GetRequiredService<ILogger<A9N.Agent.Wiki.WikiSearchIndex>>()));
        services.AddSingleton(sp => new A9N.Agent.Wiki.WikiManager(
            sp.GetRequiredService<A9N.Agent.Wiki.IWikiStorage>(),
            sp.GetRequiredService<A9N.Agent.Wiki.WikiConfig>(),
            sp.GetRequiredService<A9N.Agent.Wiki.WikiSearchIndex>(),
            sp.GetRequiredService<ILogger<A9N.Agent.Wiki.WikiManager>>()));
        ...
        var soulsSearchPaths = new[]
        {
            ...
            Path.Combine(Path.GetDirectoryName(a9nHome) ?? a9nHome, "a9n-agent", "skills", "souls"), // A9N CLI souls
        };
        ...
        services.AddSingleton(sp => new A9N.Agent.Skills.SkillInvoker(
            sp.GetRequiredService<A9N.Agent.Skills.SkillManager>(),
            sp.GetRequiredService<IChatClient>(),
            sp.GetRequiredService<ILogger<A9N.Agent.Skills.SkillInvoker>>()));

        // Chat service (pure C# — no sidecar)
        services.AddSingleton<A9NChatService>();
    ...
    {
        var agent = services.GetRequiredService<A9N.Agent.Core.Agent>();
    ...
    private static void RegisterAllTools(IServiceProvider services)
    {
        ...
        // Memory tool
        var memoryToolDir = Path.Combine(A9NEnvironment.A9NHomePath, "memories");
        RegisterAndTrack(agent, toolRegistry, new MemoryTool(memoryToolDir));

        // Session search tool
        var transcriptDir = Path.Combine(
            A9NEnvironment.A9NHomePath, "a9n-data", "transcripts");
        ...
        // Checkpoint tool
        var checkpointDir = Path.Combine(A9NEnvironment.A9NHomePath, "checkpoints");
    ...
    private static GatewayConfig BuildGatewayConfig()
    {
        ...
        var telegramToken = A9NEnvironment.ReadPlatformSetting("telegram", "token");
        ...
        var discordToken = A9NEnvironment.ReadPlatformSetting("discord", "token");
    ...
    /// are processed by the A9N agent.
    /// </summary>
    ...
    private static async void InitializeMcpAsync(IServiceProvider services, string projectDir)
    {
        try
        {
            ...
            var mcpConfigPaths = new[]
            {
                ...
                Path.Combine(A9NEnvironment.A9NHomePath, "mcp.json"),
                Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".a9n", "mcp.json")
            };
