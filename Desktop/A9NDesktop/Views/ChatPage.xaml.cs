...
public sealed partial class ChatPage : Page
{
    ...
    private readonly A9NChatService _chatService = App.Services.GetRequiredService<A9NChatService>();
    ...
    private void PopulateModelSwitcher()
    {
        ...
        var currentProvider = A9NEnvironment.ModelProvider;
        var currentModel = A9NEnvironment.DefaultModel;
        var currentBaseUrl = A9NEnvironment.ModelBaseUrl;
        var currentApiKey = A9NEnvironment.ModelApiKey ?? "";
        ...
        var anthropicKey = A9NEnvironment.ReadConfigSetting("provider_keys", "anthropic") ?? "";
        var openaiKey = A9NEnvironment.ReadConfigSetting("provider_keys", "openai") ?? "";
        var qwenKey = A9NEnvironment.ReadConfigSetting("provider_keys", "qwen") ?? "";
        var ollamaUrl = A9NEnvironment.ReadConfigSetting("provider_keys", "ollama_url") ?? "";
    ...
    private async Task SendPromptAsync()
    {
        ...
        SetBusy(true);
        ShowThinking(true, "A9N is thinking");
        ...
        dotTimer.Tick += (_, _) =>
        {
            ...
            ThinkingText.Text = $"A9N is {phase}" + new string('.', dotCount);
        };
        dotTimer.Start();
    ...
    private void PermissionModeToggle_Click(object sender, RoutedEventArgs e)
    {
        ...
        foreach (var mode in modes)
        {
            ...
            item.Click += (_, _) =>
            {
                ...
                {
                    "Plan" => A9N.Agent.Permissions.PermissionMode.Plan,
                    "Auto" => A9N.Agent.Permissions.PermissionMode.Auto,
                    "Accept Edits" => A9N.Agent.Permissions.PermissionMode.AcceptEdits,
                    "Bypass" => A9N.Agent.Permissions.PermissionMode.BypassPermissions,
                    _ => A9N.Agent.Permissions.PermissionMode.Default
                });
            };
    ...
    private void ShowOnboarding()
    {
        ...
        AppendAssistantMessage(
            ...
            "I'm A9N — your AI agent. Before we start, you can set up my identity " +
...
Write the SOUL.md content now (markdown format, start with # A9N Agent Identity):";
