...
public sealed partial class SettingsPage : Page
{
    ...
    public string A9NHomePath => A9NEnvironment.DisplayA9NHomePath;
    public string A9NConfigPath => A9NEnvironment.DisplayA9NConfigPath;
    public string A9NLogsPath => A9NEnvironment.DisplayA9NLogsPath;
    public string A9NWorkspacePath => A9NEnvironment.DisplayA9NWorkspacePath;

    public string TelegramStatus => A9NEnvironment.TelegramConfigured
        ? ResourceLoader.GetString("StatusDetected")
        : ResourceLoader.GetString("StatusNotDetected");

    public string DiscordStatus => A9NEnvironment.DiscordConfigured
        ? ResourceLoader.GetString("StatusDetected")
        : ResourceLoader.GetString("StatusNotDetected");
    ...
    {
        var userMdPath = Path.Combine(A9NEnvironment.A9NHomePath, "USER.md");
        ...
        try
        {
            ...
            UserProjectDirBox.Text = A9NEnvironment.AgentWorkingDirectory;
        }
        catch (Exception ex)
        {
...
";
            ...
            {
                Environment.SetEnvironmentVariable("A9N_DESKTOP_WORKSPACE", projectDir, EnvironmentVariableTarget.User);
            }
    ...
    {
        var provider = A9NEnvironment.ModelProvider.ToLowerInvariant();
        ...
        BaseUrlBox.Text = A9NEnvironment.ModelBaseUrl;
        ModelBox.Text = A9NEnvironment.DefaultModel;
        ApiKeyBox.Password = A9NEnvironment.ModelApiKey ?? "";

        PopulateModelCombo(provider);
        SelectCurrentModel(A9NEnvironment.DefaultModel);

        // Temperature
        var tempStr = A9NEnvironment.ReadConfigSetting("model", "temperature");
        ...
        // Max Tokens
        var maxTokStr = A9NEnvironment.ReadConfigSetting("model", "max_tokens");
    ...
    {
        var maxTurns = A9NEnvironment.ReadConfigSetting("agent", "max_turns");
        ...
        SelectComboByTag(ToolUseCombo,
            A9NEnvironment.ReadConfigSetting("agent", "tool_use") ?? "auto");
        SelectComboByTag(ApprovalsCombo,
            A9NEnvironment.ReadConfigSetting("agent", "approvals_mode") ?? "manual");

        var timeout = A9NEnvironment.ReadConfigSetting("agent", "approval_timeout");
    ...
    {
        TelegramBotTokenBox.Password = A9NEnvironment.ReadPlatformSetting("telegram", "token") ?? "";
        DiscordBotTokenBox.Password = A9NEnvironment.ReadPlatformSetting("discord", "token") ?? "";
        DiscordRequireMentionToggle.IsOn = string.Equals(
            A9NEnvironment.ReadPlatformSetting("discord", "require_mention"), "true", StringComparison.OrdinalIgnoreCase);
        DiscordAutoThreadToggle.IsOn = string.Equals(
            A9NEnvironment.ReadPlatformSetting("discord", "auto_thread"), "true", StringComparison.OrdinalIgnoreCase);
        DiscordReactionsToggle.IsOn = string.Equals(
            A9NEnvironment.ReadPlatformSetting("discord", "reactions"), "true", StringComparison.OrdinalIgnoreCase);

        SlackBotTokenBox.Password = A9NEnvironment.ReadPlatformSetting("slack", "token") ?? "";
        SlackAppTokenBox.Password = A9NEnvironment.ReadPlatformSetting("slack", "app_token") ?? "";

        MatrixAccessTokenBox.Password = A9NEnvironment.ReadPlatformSetting("matrix", "token") ?? "";
        MatrixHomeserverBox.Text = A9NEnvironment.ReadPlatformSetting("matrix", "homeserver") ?? "";

        WebhookEnabledToggle.IsOn = string.Equals(
            A9NEnvironment.ReadPlatformSetting("webhook", "enabled"), "true", StringComparison.OrdinalIgnoreCase);
        var webhookPort = A9NEnvironment.ReadPlatformSetting("webhook", "port");
        if (double.TryParse(webhookPort, NumberStyles.Integer, CultureInfo.InvariantCulture, out var wp))
            WebhookPortBox.Value = wp;
        WebhookHmacSecretBox.Password = A9NEnvironment.ReadPlatformSetting("webhook", "secret") ?? "";

        EmailAddressBox.Text = A9NEnvironment.ReadPlatformSetting("email", "address") ?? "";
        EmailPasswordBox.Password = A9NEnvironment.ReadPlatformSetting("email", "password") ?? "";
        EmailImapHostBox.Text = A9NEnvironment.ReadPlatformSetting("email", "imap_host") ?? "";
        var imapPort = A9NEnvironment.ReadPlatformSetting("email", "imap_port");
        if (double.TryParse(imapPort, NumberStyles.Integer, CultureInfo.InvariantCulture, out var ip))
            EmailImapPortBox.Value = ip;
        EmailSmtpHostBox.Text = A9NEnvironment.ReadPlatformSetting("email", "smtp_host") ?? "";
        var smtpPort = A9NEnvironment.ReadPlatformSetting("email", "smtp_port");
    ...
    private void LoadMemorySettings()
    {
        MemoryEnabledToggle.IsOn = !string.Equals(
            A9NEnvironment.ReadConfigSetting("memory", "memory_enabled"), "false", StringComparison.OrdinalIgnoreCase);
        UserProfileEnabledToggle.IsOn = !string.Equals(
            A9NEnvironment.ReadConfigSetting("memory", "user_profile_enabled"), "false", StringComparison.OrdinalIgnoreCase);

        var memLimit = A9NEnvironment.ReadConfigSetting("memory", "memory_char_limit");
        if (double.TryParse(memLimit, NumberStyles.Integer, CultureInfo.InvariantCulture, out var ml))
            MemoryCharLimitBox.Value = ml;

        var userLimit = A9NEnvironment.ReadConfigSetting("memory", "user_char_limit");
        ...
        CompressionEnabledToggle.IsOn = string.Equals(
            A9NEnvironment.ReadConfigSetting("compression", "enabled"), "true", StringComparison.OrdinalIgnoreCase);

        var threshold = A9NEnvironment.ReadConfigSetting("compression", "threshold");
    ...
    private void LoadDisplaySettings()
    {
        ShowReasoningToggle.IsOn = string.Equals(
            A9NEnvironment.ReadConfigSetting("display", "show_reasoning"), "true", StringComparison.OrdinalIgnoreCase);
        StreamingToggle.IsOn = !string.Equals(
            A9NEnvironment.ReadConfigSetting("display", "streaming"), "false", StringComparison.OrdinalIgnoreCase);
        InlineDiffsToggle.IsOn = !string.Equals(
            A9NEnvironment.ReadConfigSetting("display", "inline_diffs"), "false", StringComparison.OrdinalIgnoreCase);
        ShowCostToggle.IsOn = string.Equals(
            A9NEnvironment.ReadConfigSetting("display", "show_cost"), "true", StringComparison.OrdinalIgnoreCase);
        RedactPiiToggle.IsOn = string.Equals(
            A9NEnvironment.ReadConfigSetting("privacy", "redact_pii"), "true", StringComparison.OrdinalIgnoreCase);
    }
    ...
    private async void SaveModelConfig_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            {
                var existingKey = A9NEnvironment.ModelApiKey;
                if (!string.IsNullOrWhiteSpace(existingKey))
                    extras["api_key"] = existingKey;
            }

            await A9NEnvironment.SaveConfigSectionAsync("model", extras);
    ...
    private async void SaveAgentConfig_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            await A9NEnvironment.SaveConfigSectionAsync("agent", settings);
    ...
    private async void SavePlatformConfig_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            await A9NEnvironment.SavePlatformSettingAsync("telegram", "token", TelegramBotTokenBox.Password.Trim());

            // Discord
            await A9NEnvironment.SavePlatformSettingAsync("discord", "token", DiscordBotTokenBox.Password.Trim());
            await A9NEnvironment.SavePlatformSettingAsync("discord", "require_mention", DiscordRequireMentionToggle.IsOn.ToString().ToLowerInvariant());
            await A9NEnvironment.SavePlatformSettingAsync("discord", "auto_thread", DiscordAutoThreadToggle.IsOn.ToString().ToLowerInvariant());
            await A9NEnvironment.SavePlatformSettingAsync("discord", "reactions", DiscordReactionsToggle.IsOn.ToString().ToLowerInvariant());

            // Slack
            await A9NEnvironment.SavePlatformSettingAsync("slack", "token", SlackBotTokenBox.Password.Trim());
            await A9NEnvironment.SavePlatformSettingAsync("slack", "app_token", SlackAppTokenBox.Password.Trim());

            // Matrix
            await A9NEnvironment.SavePlatformSettingAsync("matrix", "token", MatrixAccessTokenBox.Password.Trim());
            await A9NEnvironment.SavePlatformSettingAsync("matrix", "homeserver", MatrixHomeserverBox.Text.Trim());

            // Webhook
            await A9NEnvironment.SavePlatformSettingAsync("webhook", "enabled", WebhookEnabledToggle.IsOn.ToString().ToLowerInvariant());
            await A9NEnvironment.SavePlatformSettingAsync("webhook", "port", ((int)WebhookPortBox.Value).ToString(CultureInfo.InvariantCulture));
            await A9NEnvironment.SavePlatformSettingAsync("webhook", "secret", WebhookHmacSecretBox.Password.Trim());

            // Email
            await A9NEnvironment.SavePlatformSettingAsync("email", "address", EmailAddressBox.Text.Trim());
            await A9NEnvironment.SavePlatformSettingAsync("email", "password", EmailPasswordBox.Password.Trim());
            await A9NEnvironment.SavePlatformSettingAsync("email", "imap_host", EmailImapHostBox.Text.Trim());
            await A9NEnvironment.SavePlatformSettingAsync("email", "imap_port", ((int)EmailImapPortBox.Value).ToString(CultureInfo.InvariantCulture));
            await A9NEnvironment.SavePlatformSettingAsync("email", "smtp_host", EmailSmtpHostBox.Text.Trim());
            await A9NEnvironment.SavePlatformSettingAsync("email", "smtp_port", ((int)EmailSmtpPortBox.Value).ToString(CultureInfo.InvariantCulture));
    ...
    private async void SaveMemoryConfig_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            await A9NEnvironment.SaveConfigSectionAsync("memory", memSettings);
            ...
            await A9NEnvironment.SaveConfigSectionAsync("compression", compSettings);
    ...
    private async void SaveDisplayConfig_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            await A9NEnvironment.SaveConfigSectionAsync("display", displaySettings);

            var privacySettings = new Dictionary<string, string>
            {
                ["redact_pii"] = RedactPiiToggle.IsOn.ToString().ToLowerInvariant(),
            };
            await A9NEnvironment.SaveConfigSectionAsync("privacy", privacySettings);
    ...
    // ═══════════════════════════════════════════
    private void OpenHome_Click(object sender, RoutedEventArgs e) => A9NEnvironment.OpenA9NHome();
    private void OpenConfig_Click(object sender, RoutedEventArgs e) => A9NEnvironment.OpenConfig();
    private void OpenLogs_Click(object sender, RoutedEventArgs e) => A9NEnvironment.OpenLogs();
    private void OpenWorkspace_Click(object sender, RoutedEventArgs e) => A9NEnvironment.OpenWorkspace();
    ...
    private void LoadExecutionSettings()
    {
        SelectComboByTag(ExecBackendCombo,
            A9NEnvironment.ReadConfigSetting("terminal", "backend") ?? "local");

        ExecWorkingDirBox.Text = A9NEnvironment.ReadConfigSetting("terminal", "working_directory") ?? ".";

        var timeout = A9NEnvironment.ReadConfigSetting("terminal", "timeout");
        if (double.TryParse(timeout, NumberStyles.Integer, CultureInfo.InvariantCulture, out var to))
            ExecTimeoutBox.Value = to;

        DockerImageBox.Text = A9NEnvironment.ReadConfigSetting("terminal", "docker_image")
            ?? "nikolaik/python-nodejs:python3.11-nodejs20";

        var cpu = A9NEnvironment.ReadConfigSetting("terminal", "container_cpu");
        if (double.TryParse(cpu, NumberStyles.Integer, CultureInfo.InvariantCulture, out var c))
            DockerCpuBox.Value = c;

        var mem = A9NEnvironment.ReadConfigSetting("terminal", "container_memory");
    ...
    private async void SaveExecutionConfig_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            await A9NEnvironment.SaveConfigSectionAsync("terminal", settings);
    ...
    {
        var builtinEnabled = A9NEnvironment.ReadConfigSetting("plugins", "builtin_memory");
        BuiltinMemoryPluginToggle.IsOn = !string.Equals(builtinEnabled, "false", StringComparison.OrdinalIgnoreCase);

        ExternalMemoryProviderBox.Text = A9NEnvironment.ReadConfigSetting("plugins", "external_memory_provider") ?? "";
    }
    ...
    private async void SavePluginConfig_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            await A9NEnvironment.SaveConfigSectionAsync("plugins", settings);
