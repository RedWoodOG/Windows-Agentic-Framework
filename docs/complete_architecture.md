# A9N - Complete Architecture Blueprint
...
A9N/
├── src/
│   ├── A9N.Agent.csproj          # Core library
│   ├── A9N.CLI.csproj            # CLI executable
│   ├── A9N.Gateway.csproj        # Multi-platform gateway (Telegram, Discord)
│   │
│   ├── Core/
...
namespace A9N.Agent.Transcript;
...
public sealed class TranscriptStore
{
    ...
    private bool ShouldEagerFlush()
    {
        ...
        // 1. A9N_EAGER_FLUSH env var set
        // 2. Running in Cowork (cloud) mode
        // 3. Bare mode (--bare flag) - fire-and-forget, no flush
        return Environment.GetEnvironmentVariable("A9N_EAGER_FLUSH") != null;
    }
}
...
namespace A9N.Agent.Permissions;
...
Once these 4 are done, A9N will have capabilities matching modern agentic systems.
