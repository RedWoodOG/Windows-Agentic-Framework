# Upstream Tracking: Architecture Reference

## Current Parity
- **Upstream version:** v0.7.0 (2026.4.3)
- **Last synced:** 2026-04-04
- **Note:** This project is an independent Windows agentic framework
- **Upstream install path:** `%LOCALAPPDATA%\a9n\a9n-agent`

## Architecture Mapping

### Python Module → C# Equivalent

| Upstream (Python) | A9N (C#) | Status |
|---|---|---|
...
| `cli.py` + `a9n_cli/` | Desktop UI (WinUI 3) | ✅ Reimplemented as native desktop |
...
| `gateway/` | `Desktop/A9NDesktop/Views/IntegrationsPage.xaml` | 🟡 Display only (no gateway daemon) |
...
| `a9n_cli/skin_engine.py` | `App.xaml` theme system | ✅ Reimplemented as XAML resources |
| `a9n_cli/banner.py` | `DashboardPage.xaml` hero section | ✅ Reimplemented |
| `src/buddy/buddy.cs` | `src/buddy/buddy.cs` | ✅ C# original (native implementation) |
| `src/coordinator/` | `src/coordinator/coordinatorservice.cs` | ✅ C# original (native implementation) |
...
8. **API Server Session Continuity** — persistent sessions with X-A9N-Session-Id
...
13. Fork detection in `a9n update`
...
#   %LOCALAPPDATA%\a9n\a9n-agent\RELEASE_v{VERSION}.md

# 3. Diff the Python source against our mapping table above
# Focus on: agent/, tools/, a9n_cli/config.py, toolsets.py
...
These are features unique to A9N Desktop:
