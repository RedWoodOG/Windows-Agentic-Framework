...
**Date**: 2026-04-03
**Purpose**: Complete specification of KAIROS proactive mode and multi-agent orchestration for A9N
...
function matchSessionMode(
  ...
  if (currentMode !== sessionMode) {
    ...
    if (sessionMode === 'coordinator') {
      process.env.A9N_COORDINATOR_MODE = 'true';
    } else {
      delete process.env.A9N_COORDINATOR_MODE;
    }
