...
**C# port**: `src/LLM/ModelRouter.cs`. Simple method `ShouldUseCheapModel(string message) -> bool`. Plug into `A9NChatService` before `StreamStructuredAsync`.
...
**C# port**: Existing `src/hooks/HookSystem.cs` already has 9 event types. Extend with discovery from `~/.a9n-data/hooks/` directories.
