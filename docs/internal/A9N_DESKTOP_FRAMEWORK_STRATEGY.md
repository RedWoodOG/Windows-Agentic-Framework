# A9N Desktop — Native Windows Agentic Framework
...
**Decision:** A9N Desktop is refocused to be a **native Windows agentic framework and product**, not a downstream reimplementation of upstream A9N.

**Facts we accept:**

- Upstream A9N bundles **multiple products** (TUI, gateway/server, Python runtime ecosystem). Those are **different form factors and lifetimes** than a WinUI desktop app.
...
- Treating upstream A9N as a **checklist to complete**.
...
- **Pages map to mental models:** Chat (work), Sessions (structure), Replay/Activity (audit), Skills (capabilities), Memory (long-term), Settings (trust and cost), Agent/Soul (identity). Existing XAML layout under [`Desktop/A9NDesktop/Views/`](../../Desktop/A9NDesktop/Views/) should stay **task-centered**, not "settings for everything."
...
- Sessions are the **unit of work** on desktop: branch conversations, resume, export. Tie UI ([`SessionPanel`](../../Desktop/A9NDesktop/Views/Panels/SessionPanel.xaml)) to transcript IDs and persistence guarantees.
...
| **Deliverables** | Manual E2E script + captured evidence (session id, transcript snippet, screenshot or log); fix regressions in [`A9NChatService`](../../Desktop/A9NDesktop/Services/A9NChatService.cs) / [`AnthropicClient`](../../src/LLM/AnthropicClient.cs) / [`Agent`](../../src/Core/agent.cs) as needed. |
...
| **Persistence** | Corruption, ordering | Power loss simulation; `A9N_EAGER_FLUSH` testing | Truncated JSONL, unrecoverable sessions |
...
| P0 | Streaming vs non-streaming parity audit (tool turns) | Agent, A9NChatService | Prevents split-brain bugs | Immediate E2E | Automated or scripted dual-path test |
...
A9N Desktop is a **solid, trustworthy, Windows-native agentic framework and desktop product**: strong runtime foundations, strong native UX, and **no trap of upstream imitation.**
